...
def on_message(client, userdata, msg):
    ...
    if "turn on light" in message.lower():
        ...
        # Send command to ESP NodeMCU to turn on the first light (you need to implement this)
        client.publish(MQTT_TOPIC, "turn on light 1")
        # Additionally, send command to ESP NodeMCU to turn on the second light (pin 4)
        client.publish(MQTT_TOPIC, "turn on light 2")

    elif "turn off light" in message.lower():
        speak("Turning off the lights...")
        # Send command to ESP NodeMCU to turn off the first light (you need to implement this)
        client.publish(MQTT_TOPIC, "turn off light 1")
        # Additionally, send command to ESP NodeMCU to turn off the second light (pin 4)
        client.publish(MQTT_TOPIC, "turn off light 2")
